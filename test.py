...
def break_in_grids_of_16(string):
    all = []
    for i in range(len(string) // 16):
        b = string[i * 16 : (i * 16) + 16]
        grid = [[], [], [], []]
        for i in range(4):
            for j in range(4):
                print(f"b[i + (j * 4)]: {b[i + (j * 4)]}")
                grid[i].append(ord(b[i + (j * 4)]))
        all.append(grid)

    print(f"str: {string}")
    print(f"break_in_grids_of_16: {all}")
    return all
...
print(expand_key("bonjour je mange", 10))
